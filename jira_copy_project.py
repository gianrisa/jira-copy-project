...
def jissue_field_prepare_dummy_s(project, inst):
...
def __epic(jinst):
    if jinst:
        ...
        __epic = 'customfield_10004' # pre instance 
    return __epic
...
def phase(issue):
    if issuetype_map[issue.fields.issuetype.name] == 'Defect':
        try:
            phase = issue.fields.customfield_10200.value
        except:
...
def __phase(jinst):
    if jinst:
        ...
        __phase = 'customfield_10200' # pre instance 
    return __phase
...
@timeit
def copy_issues(jira_in, jira_out, project, issue_max_key, issues_old, inst, start=0):
        ...
        if i >= start:
            ...
            if issue:
                ...
                try:
                    ...
                    issue_dict = jissue_field_prepare_mapped(issue, jira_out, project, inst)
...
def main():
    ...
    parser.add_option("-P", "--production",  help="copy to production",
                      action="store_true", dest="inst", default=False)
